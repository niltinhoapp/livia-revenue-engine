...
async function searchLeads(event) {
  ...
  try {
    ...
    state.leads = Array.isArray(data.leads) ? data.leads : []; const contacted = readContacted(); state.leads.forEach((lead) => { if (contacted.has(String(lead.id))) lead.stage = 'CONTATADO'; });
...
function updateSummary(data) { $('metric-found').textContent = data.collected ?? state.leads.length; $('metric-qualified').textContent = data.qualified ?? state.leads.filter((lead) => lead.stage === 'QUALIFICADO').length; $('metric-phone').textContent = state.leads.filter((lead) => Boolean(lead.phone)).length; const found = Number(data.collected ?? state.leads.length); const qualified = Number(data.qualified ?? 0); $('metric-rate').textContent = found ? `${Math.round((qualified / found) * 100)}%` : '0%'; selectedSegmentLabel.textContent = segmentNames[data.segment] || segmentNames[searchSegment.value] || 'Segmento selecionado'; }
function renderTable() { const segment = segmentFilter.value; const minimumScore = Number(scoreFilter.value); state.filtered = state.leads.filter((lead) => (segment === 'all' || lead.segment === segment) && Number(lead.score || 0) >= minimumScore); table.innerHTML = state.filtered.map((lead) => `<tr><td><div class="company">${escapeHtml(lead.name)}</div><div class="muted">${escapeHtml([lead.city, lead.state].filter(Boolean).join(' · '))}</div></td><td><span class="segment-label">${escapeHtml(segmentNames[lead.segment] || 'Outro')}</span></td><td><span class="${scoreClass(Number(lead.score || 0))}">${Number(lead.score || 0)}</span></td><td>${escapeHtml(formatPhone(lead.phone))}</td><td><span class="stage">${escapeHtml(isContacted(lead) ? stageNames.CONTATADO : (stageNames[lead.stage] || lead.stage || '—'))}</span></td><td><button class="details-btn" data-lead-id="${escapeHtml(lead.id)}">Ver lead</button></td></tr>`).join(''); empty.classList.toggle('hidden', state.filtered.length !== 0); $('results-subtitle').textContent = `${state.filtered.length} lead${state.filtered.length === 1 ? '' : 's'} exibido${state.filtered.length === 1 ? '' : 's'} · ${segmentNames[searchSegment.value] || 'segmento'} · resultado da última busca`; }
...
function openLead(id) {
  ...
  drawerContent.innerHTML = `<div class="detail-eyebrow">Lead · ${escapeHtml(isContacted(lead) ? stageNames.CONTATADO : (stageNames[lead.stage] || lead.stage || 'Novo'))}</div><h2 class="detail-title">${escapeHtml(lead.name)}</h2><span class="detail-score">Score ${Number(lead.score || 0)}</span><div class="detail-grid"><div class="detail-item"><span>Segmento</span><strong>${escapeHtml(segmentNames[lead.segment] || 'Não identificado')}</strong></div><div class="detail-item"><span>Avaliação</span><strong>${escapeHtml(formatRating(lead.rating, lead.reviews))}</strong></div><div class="detail-item"><span>Telefone</span><strong>${escapeHtml(formatPhone(lead.phone))}</strong></div><div class="detail-item"><span>Cidade</span><strong>${escapeHtml([lead.city, lead.state].filter(Boolean).join(' · ') || '—')}</strong></div></div>${renderPhoneVerification(lead)}<div class="detail-section"><h3>Por que foi qualificado</h3>${reasons}</div><div class="detail-section"><h3>Endereço</h3><div class="muted">${escapeHtml(lead.address || 'Não informado')}</div></div><div class="detail-section"><h3>Links</h3><div class="detail-actions">${lead.googleMapsUrl ? `<a class="secondary" href="${escapeHtml(lead.googleMapsUrl)}" target="_blank" rel="noopener noreferrer">Abrir no Maps</a>` : ''}${lead.website ? `<a class="secondary" href="${escapeHtml(lead.website)}" target="_blank" rel="noopener noreferrer">Abrir site</a>` : ''}</div></div>${renderApproach(lead)}${renderCrmHistory(lead)}`;
